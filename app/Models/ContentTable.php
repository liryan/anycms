...
class ContentTable extends BaseSetting
{
	 ...
	 * @param  [type]  $search       [Array('condition'=>string,'order'=>string]
	...
	public function getList($start,$length,$table_define,$id,$search=[])
	{
		...
		$count=DB::table($table_define['name'])->whereRaw($search['condition'])->count();
		$data=$query->whereRaw($search['condition'])->orderByRaw($search['order'])->skip($start)->take($length)->get();
		if($data){
			$data=$data->toArray();
		}
